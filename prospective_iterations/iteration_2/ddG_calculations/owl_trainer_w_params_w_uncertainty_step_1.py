...
script_folder_uncert=f"{pocket_fit_folder}/prediction_step_1"
...
sys.path.append(script_folder_uncert)
...
from custom_dataset_modular_with_uncert import CustomMolModularDataset_w_uncert
...
def train_model_with_settings_and_uncertainty(settings_fname, redo=False, device=torch.device("cpu"), show_progress=False,
                              ...
                              save_folder_base=f"{pocket_fit_folder}/prediction_step_1/uncertainty_test/"):
    print("\n\n\n")
    ...
    print("# of training ligands found:", len(ligs), datetime.now())
    #raise()
    ...
    print("# starting ligands:",n_starting_ligs, datetime.now())
    ...
    print("Finished setting up weights", datetime.now())
    ...
    # do the training
    #################
    print("Start setting up models", datetime.now())
    ...
    for drop in range(XVal_blocks):
        ...
        Train_set = CustomMolModularDataset_w_uncert([ligs[idx] for idx in Train_indeces], no_core_ligs=[no_core_ligs[idx] for idx in Train_indeces],
                                            representation_flags=representation_flags, normalize_x=normalize_x, X_filter=X_filter, datafolder=datafolder)
        XVal_set  = CustomMolModularDataset_w_uncert([ligs[idx] for idx in XVal_indeces ], no_core_ligs=[no_core_ligs[idx] for idx in Train_indeces],
                                            representation_flags=representation_flags, normalize_x=normalize_x, X_filter=X_filter, datafolder=datafolder)
...
if __name__== "__main__":
    ...
    for f in args.settings_fname:
        try:
            train_model_with_settings_and_uncertainty(f, args.redo, device, args.show_progress, datafolder=args.datafolder)
