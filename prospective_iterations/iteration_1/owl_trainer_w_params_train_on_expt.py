...
folder=f"{pocket_fit_folder}/prediction_step_0/repr_scan_step_0_on_preliminary_calc_ddG/meta_param_search_train_on_expt_ligs"
...
sys.path.insert(0, pocket_fit_folder)
...
def train_model_on_expt_with_settings(settings_fname, redo=False, device=torch.device("cpu"), show_progress=False,
    ...
    save_folder=folder+f"/models/{dr_name}/"
    ...
    for drop in range(XVal_blocks):
        ...
        #print(Train_set[0])
        #print(ligs[Train_indeces[0]].GetProp('ID'))
        #print(ligs[Train_indeces[0]].GetProp('dG'))
        #raise()
...
if __name__== "__main__":
    ...
    for f in args.settings_fname:
        try:
            train_model_on_expt_with_settings(f, args.redo, device, args.show_progress, datafolder=args.datafolder)
