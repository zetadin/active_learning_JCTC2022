...
#                raise(Exception("need mdenerg_ranges specified for binning"))
    ...
    def generate_DataBlock(self, lig, blockID):
        ...
        # ESP extras
            ...
            key=lig.GetProp('ID')
            fns=glob.glob(f"{self.morphes_sim_an_folder}/{key}/energy_block_?.xvg")
            vals=[]
            for i in range(len(fns)):
                fn=f"{self.morphes_sim_an_folder}/{key}/energy_block_{i+1}.xvg"
                if(not os.path.exists(fn)):
                    raise(Exception(f"ligand {key}: missing energy_block_{i+1}.xvg"))
